...
if ! [ -f "${Subc}" ]; then
 ...
 if [[ ${useFreesurfer} -eq 1 ]]; then
  bash CreateSubcortical.sh "${DataFolder}" "${OutFolder}" -f "${FS_DIR}"
 else
  bash CreateSubcortical.sh "${DataFolder}" "${OutFolder}"
 fi

fi

# Register Subcortical mask to fMRI space
flirt -in "${Subc}" -ref "${OutFolder}"/mean.nii.gz -applyxfm -init "${DataFolder}"/anat2func.mat -interp nearestneighbour -o "${OutFolder}"/subc_fmri.nii.gz
...
# Create Label file
wb_command -volume-label-import "${OutFolder}"/subc_fmri.nii.gz "${OutFolder}"/subcortical-structures.txt "${OutFolder}"/subc_fmri.nii.gz -discard-others -drop-unused-labels


if [[ $(echo 3 == 3 | bc -l | cut -f1 -d.) == "1" ]]
then
    echo "Creating subject-roi subcortical cifti at same resolution as output"
    wb_command -cifti-create-dense-timeseries "${OutFolder}"/func_temp_subject.dtseries.nii -volume "${DataFolder}"/preproc_rest.nii.gz "${OutFolder}"/subc_fmri.nii.gz
else
...
wb_command -cifti-dilate "${OutFolder}"/func_temp_subject.dtseries.nii COLUMN 0 30 "${OutFolder}"/func_temp_subject_dilate.dtseries.nii
# write output volume
wb_command -cifti-separate "${OutFolder}"/func_temp_subject_dilate.dtseries.nii COLUMN -volume-all "${OutFolder}"/subcortical_fmri.nii.gz
